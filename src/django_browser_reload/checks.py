...
def check_django_browser_reload_setup(**kwargs):
    ...
    if "django_browser_reload.middleware.BrowserReloadMiddleware" not in settings.MIDDLEWARE:
        errors.append(Error(
            "The 'BrowserReloadMiddleware' must be added to MIDDLEWARE.",
            id="django_browser_reload.E002",
        ))
    if "django.contrib.staticfiles.finders.AppDirectoriesFinder" not in getattr(settings, "STATICFILES_FINDERS", []):
        errors.append(Error(
